...
def wta_hyperparam_search(a_in, 
    ...
    n_steps = n_steps
    # Activation inputs to be distinguished in the temporal encoding by the spike times
    a_in = a_in 
    ...
    for i in range(num_neurons):
        print(f"Spike time for neuron {i + 1} with a_in={a_in[i]}: {best_spike_times[i]}")
...
def plot_wta_voltage_dynamics(v_hist, vth):
    ...
    plt.step(np.arange(n_steps+1), np.vstack((np.zeros((1,n_input)),v_hist)))
    ax=plt.gca()
    ax.hlines(y=vth, xmin=0, xmax=n_steps, linewidth=3, color='grey', linestyle='--')
    for t in spike_times:
        ax.vlines(x=t, ymin=v_min, ymax=vth, linewidth=2, color='lightgrey',linestyle='--')
    plt.xticks(np.arange(n_steps), size=15)
    plt.yticks(np.arange(np.round(v_min,-3), np.round(1.05*v_max,-3), np.round((1.05*v_max-v_min)/10,-3)), size=15)
    ...
    plt.title("Voltage dynamics of LIF neuron tuned for temporal WTA", fontsize=20)
    plt.show()
