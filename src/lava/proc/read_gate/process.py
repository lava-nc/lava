...
class ReadGate(AbstractProcess):
    ...
    ----------
    shape : tuple(int)
        The shape of the set of units in the downstream process whose state
    ...
    --------
    cost_out
        Forwards to an upstream process the better cost notified by the
        CostIntegrator.
    solution_out
        Forwards to an upstream process the better variable assignment
