...
class PyLifModelBitAcc(PyLoihiProcessModel):
    ...
    def run_spk(self):
        ...
        if not self.b_vth_computed:
            ...
            self.effective_vth = np.left_shift(self.vth, 6)
        ...
        decayed_curr = np.int64(self.u) * (4096 - decay_const_u)
        ...
        wrapped_curr = np.mod(decayed_curr, np.sign(decayed_curr) * (2 ** 23))
        self.u[:] = wrapped_curr
        # Update voltage
        # --------------
        ...
        #  limits)
        neg_voltage_limit = -np.int32(2 ** 23) + 1
        pos_voltage_limit = np.int32(2 ** 23) - 1
        # Decaying voltage similar to current. See the comment above to
        # understand the need for each of the operations below.
        decayed_volt = np.int64(self.v) * (4096 - decay_const_v)
