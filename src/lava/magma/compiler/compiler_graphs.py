...
from lava.magma.core.process.ports.ports import AbstractPort, RefPort, InPort, \
    VarPort, OutPort, AbstractVirtualPort
...
class ProcDiGraph(DiGraphBase):
    ...
    def _traverse_ports_of_proc(proc: AbstractProcess) -> ty.Tuple[
        ...
        in_proc_list = \
            ProcDiGraph._find_terminal_procs_recursively(port_list_in, "inward")
...
class ProcGroupDiGraphs(AbstractProcGroupDiGraphs):
    ...
    def _map_proc_to_model(procs: ty.List[AbstractProcess],
        ...
        for proc in procs:
            ...
            model_cls = \
                ProcGroupDiGraphs._select_proc_models(proc, models_cls, run_cfg)
