...
connections_multi_peak = Syn(weights=weights_multi_peak, synname="connections_multi_peak")
sparse_2 = Syn(weights=np.eye(num_neurons) * sparse2_weights, synname="sparse_2")
dnf_selective = LIF(shape=down_sampled_shape,
                    du=du_selective,
                    dv=dv_selective,
                    vth=selective_threshold)
connections_selective = Syn(weights=weights_selective, synname="connections_selective")
...
dnf_multi_peak.s_out.reshape(new_shape=down_sampled_flat_shape).connect(
    sparse_2.s_in) #* down_sampled_flat_shape: (num_neurons,)
...
sparse_1.a_out.reshape(new_shape=down_sampled_shape).connect(rate_reader_multi_peak.in_port)
