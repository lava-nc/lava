...
class SimplePort {
 public:
  ...
  void transfer() {
    ...
    // if (metadata->type == 'i') {
    if (1) {
      int *data = reinterpret_cast<int*> (this->metadata->mdata);
      for (int i = 0; i < this->metadata->dimensions[0] \
                    *this->metadata->strides[0]/sizeof(int); i++) {
        data[i]++;
      }
    }
...
class ProxySimplePort {
 public:
  ...
  py::object* set_data(py::object *obj) {
    ...
    std::cout << (uint64_t)this->obj << std::endl;
    return this->obj;
  }
  py::object* get_data() {
