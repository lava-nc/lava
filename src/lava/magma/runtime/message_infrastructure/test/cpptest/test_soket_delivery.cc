...
void soket_target_fn1(
  ...
  AbstractActor* actor_ptr) {
    ...
    to_a2->Start();
    from_a2->Start();
    // LAVA_DUMP(1, "socket actor1, loop: %d\n", loop);
    while ((loop--)&&!actor_ptr->GetStatus()) {
      // LAVA_DUMP(1, "soket actor1 waitting\n");
      MetaDataPtr data = from_mp->Recv();
      // LAVA_DUMP(1, "socket actor1 recviced\n");
...
void soket_target_fn2(
  ...
  AbstractActor* actor_ptr) {
    ...
    while ((loop--)&&!actor_ptr->GetStatus()) {
      // LAVA_DUMP(1, "socket actor2 waitting\n");
      MetaDataPtr data = from_a1->Recv();
      // LAVA_DUMP(1, "socket actor2 recviced\n");
      (*reinterpret_cast<int64_t*>(data->mdata))++;
      to_a1->Send(data);
      free(reinterpret_cast<char*>(data->mdata));
    }
...
TEST(TestSocketChannel, SocketLoop) {
  ...
  while (loop--) {
    ...
    // for (int i = 0; i < 20; i++) {
    //   LAVA_DUMP(1, "socket mdata: %p, socket *mdata: %ld\n", ptr, *ptr);
    //   ptr++;
    // }
  ...
  LAVA_DUMP(1, "socket cpp loop timedelta: %f",
           ((end_time - start_time)/static_cast<double>(CLOCKS_PER_SEC)));
  LAVA_DUMP(1, "exit\n");
}
