...
class P1(AbstractProcess):
    def __init__(self, **kwargs):
        ...
        self.v = Var(shape=(2,), init=17)
        self.var_port_read = VarPort(self.v)
...
class P2(AbstractProcess):
    def __init__(self, **kwargs):
        ...
        self.ref_read = RefPort(shape=(2,))
        self.var_read = Var(shape=(2,), init=1)
...
class PyProcModel1(PyLoihiProcessModel):
    ...
    var_port_read: PyVarPort = LavaPyType(PyVarPort.VEC_DENSE, int)
...
class PyProcModel2(PyLoihiProcessModel):
    ref_read: PyRefPort = LavaPyType(PyRefPort.VEC_DENSE, int)
    var: np.ndarray = LavaPyType(np.ndarray, np.int32)
    var_port: PyVarPort = LavaPyType(PyVarPort.VEC_DENSE, int)
    var_read: np.ndarray = LavaPyType(np.ndarray, np.int32)
    ...
    def run_pre_mgmt(self):
        if self.current_ts > 1:
            self.var_read = self.ref_read.read()
...
class TestRefVarPorts(unittest.TestCase):
    ...
    def test_explicit_Ref_Var_port_write(self):
        ...
        sender.ref.connect(recv.var_port)
        ...
        self.assertTrue(np.all(recv.var.get() == np.array([4., 4., 4.])))
        # second time step, data is sent (7)
        sender.run(RunSteps(num_steps=1, blocking=True),
                   MyRunCfg(custom_sync_domains=[simple_sync_domain]))
        self.assertTrue(np.all(recv.var.get() == np.array([7., 7., 7.])))
        # third time step, data is sent (8)
        sender.run(RunSteps(num_steps=1, blocking=True),
                   MyRunCfg(custom_sync_domains=[simple_sync_domain]))
        self.assertTrue(np.all(recv.var.get() == np.array([8., 8., 8.])))
        # fourth time step, data is sent (9)
        sender.run(RunSteps(num_steps=1, blocking=True),
                   MyRunCfg(custom_sync_domains=[simple_sync_domain]))
        self.assertTrue(np.all(recv.var.get() == np.array([9., 9., 9.])))
        sender.stop()
    ...
    def test_implicit_Ref_Var_port_write(self):
        ...
        sender.ref.connect_var(recv.var)
    ...
    def test_explicit_Ref_Var_port_read(self):
        ...
        # Connect RefPort with explicit VarPort
        recv.ref_read.connect(sender.var_port_read)
        ...
        self.assertTrue(np.all(recv.var_read.get() == np.array([1., 1.])))
        # second time step, the RefPort read from the VarPort and wrote the
        # result in "var_read" (= 17)
        sender.run(RunSteps(num_steps=1, blocking=True),
                   MyRunCfg(custom_sync_domains=[simple_sync_domain]))
        self.assertTrue(
            np.all(recv.var_read.get() == np.array([17., 17.])))
        sender.stop()
    ...
    def test_implicit_Ref_Var_port_read(self):
        ...
        # Connect RefPort with explicit VarPort
        recv.ref_read.connect_var(sender.v)
