...
from lava.magma.core.learning.learning_rule import LoihiLearningRule, Loihi2FLearningRule, Loihi3FLearningRule
...
def create_network(size, num_steps, t_spike, learning_rule, weights_init, target="pre", precision="floating_pt"):
        ...
        if isinstance(learning_rule, Loihi2FLearningRule):
            lif_1 = LIF(shape=(size,),
                        ...
                        bias_mant=0.0,
                        name="lif_post")
        elif isinstance(learning_rule, Loihi3FLearningRule):
...
class TestLoihiLearningRule(unittest.TestCase):
    ...
    def test_get_set_y1_tau_float(self) -> None:
        """Tests changing y1_tau during runtime in a floating point simulation"""
        ...
        _, _, _, dense, lif_1 = create_network(size, 
                                           num_steps, 
                                           t_spike, 
                                           learning_rule, 
                                           weights_init,
        ...
        assert(y1[t_spike + num_steps + 2] == y1_impulse * np.exp(-1/y1_tau_new))


    def test_get_set_y1_impulse_float(self) -> None:
        """Tests changing y1_impulse during runtime in a floating point simulation"""
    ...
    def test_get_set_y2_tau_float(self) -> None:
        """Tests changing y2_tau during runtime in a floating point simulation"""
        ...
        assert(y2[t_spike + num_steps + 2] == y2_impulse * np.exp(-1/y2_tau_new))


    def test_get_set_y2_impulse_float(self) -> None:
        """Tests changing y2_impulse during runtime in a floating point simulation"""
    ...
    def test_get_set_y3_tau_float(self) -> None:
        """Tests changing y3_tau during runtime in a floating point simulation"""
        ...
        assert(y3[t_spike + num_steps + 2] == y3_impulse * np.exp(-1/y3_tau_new))


    def test_get_set_y3_impulse_float(self) -> None:
        """Tests changing y3_impulse during runtime in a floating point simulation"""
    ...
    def test_get_set_dw_float(self) -> None:
        ...
        learning_rule = Loihi2FLearningRule(dw=dw_init,
                                            t_epoch=t_epoch)
    ...
    def test_get_set_dt_float(self) -> None:
        ...
        learning_rule = Loihi2FLearningRule(dt=dt_init,
                                            t_epoch=t_epoch)
    ...
    def test_get_set_dd_float(self) -> None:
        ...
        learning_rule = Loihi2FLearningRule(dd=dd_init,
                                            t_epoch=t_epoch)
    ...
    def test_get_set_x1_tau_fixed(self) -> None:
        """Tests changing x1_tau during runtime in a fixed point simulation"""
        ...
        assert(x1[t_spike + 1] == x1_impulse)
        assert(np.abs(x1[t_spike + 2] - x1_impulse * np.exp(-1/x1_tau_init)) < 1)

        assert(x1[t_spike + num_steps + 1] == x1_impulse)
        assert(np.abs(x1[t_spike + num_steps + 2] - x1_impulse * np.exp(-1/x1_tau_new)) < 1)


    def test_get_set_x1_impulse_fixed(self) -> None:
        """Tests changing x1_impulse during runtime in a fixed point simulation"""
    ...
    def test_get_set_x2_tau_fixed(self) -> None:
        """Tests changing x2_tau during runtime in a fixed point simulation"""
        ...
        assert(x2[t_spike + 1] == x2_impulse)
        assert(np.abs(x2[t_spike + 2] - x2_impulse * np.exp(-1/x2_tau_init)) < 1)

        assert(x2[t_spike + num_steps + 1] == x2_impulse)
        assert(np.abs(x2[t_spike + num_steps + 2] - x2_impulse * np.exp(-1/x2_tau_new)) < 1)


    def test_get_set_x2_impulse_fixed(self) -> None:
        """Tests changing x2_impulse during runtime in a fixed point simulation"""
    ...
    def test_get_set_y1_tau_fixed(self) -> None:
        """Tests changing y1_tau during runtime in a fixed point simulation"""
        ...
        assert(y1[t_spike + 1] == y1_impulse)
        assert(np.abs(y1[t_spike + 2] - y1_impulse * np.exp(-1/y1_tau_init)) < 1)

        assert(y1[t_spike + num_steps + 1] == y1_impulse)
        assert(np.abs(y1[t_spike + num_steps + 2] - y1_impulse * np.exp(-1/y1_tau_new)) < 1)


    def test_get_set_y1_impulse_fixed(self) -> None:
        """Tests changing y1_impulse during runtime in a fixed point simulation"""
    ...
    def test_get_set_y2_tau_fixed(self) -> None:
        """Tests changing y2_tau during runtime in a fixed point simulation"""
        ...
        assert(y2[t_spike + 1] == y2_impulse)
        assert(np.abs(y2[t_spike + 2] - y2_impulse * np.exp(-1/y2_tau_init)) < 1)

        assert(y2[t_spike + num_steps + 1] == y2_impulse)
        assert(np.abs(y2[t_spike + num_steps + 2] - y2_impulse * np.exp(-1/y2_tau_new)) < 1)


    def test_get_set_y2_impulse_fixed(self) -> None:
        """Tests changing y2_impulse during runtime in a fixed point simulation"""
    ...
    def test_get_set_y3_tau_fixed(self) -> None:
        """Tests changing y3_tau during runtime in a fixed point simulation"""
        ...
        assert(y3[t_spike + 1] == y3_impulse)
        assert(np.abs(y3[t_spike + 2] - y3_impulse * np.exp(-1/y3_tau_init)) < 1)

        assert(y3[t_spike + num_steps + 1] == y3_impulse)
        assert(np.abs(y3[t_spike + num_steps + 2] - y3_impulse * np.exp(-1/y3_tau_new)) < 1)


    def test_get_set_y3_impulse_fixed(self) -> None:
        """Tests changing y3_impulse during runtime in a fixed point simulation"""
    ...
    def test_get_set_dw_fixed(self) -> None:
        """Tests changing dw during runtime in a fixed point simulation"""
    ...
    def test_get_set_dt_fixed(self) -> None:
        """Tests changing dt during runtime in a fixed point simulation"""
    ...
    def test_get_set_dd_fixed(self) -> None:
        """Tests changing dd during runtime in a fixed point simulation"""
