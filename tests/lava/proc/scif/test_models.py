...
class TestSCIFModels(unittest.TestCase):
    ...
    def run_test(
        ...
        dense_in.a_out.connect(scif.a_in)
        scif.s_wta_out.connect(dense_wta.s_in)
        scif.s_sig_out.connect(dense_sig.s_in)
        ...
        for j in range(num_steps):
            ...
            scif.run(condition=run_condition, run_cfg=run_config)
            spk_src.data.set(np.array([[0] * num_neurons]).astype(int))
            volts_scif.append(scif.v.get())
    ...
    def test_scif_fixed_pt_no_noise(self) -> None:
        ...
        neg_tau_ref = -5
        wt = 2
        total_period = theta // bias - neg_tau_ref
        num_epochs = 10
        num_steps = num_epochs * total_period + (theta // bias)
        ...
        #                      v_lif_sig))
        # print(voltages)
        spk_idxs = np.array([theta // bias - 1 + j * total_period for j in
    ...
    def test_scif_fp_no_noise_interrupt_rfct_mid(self) -> None:
        ...
        total_period = (theta // bias) - neg_tau_ref
        num_epochs = 5
        num_steps = (theta // bias) + num_epochs * total_period + inj_times[1]
        v_scif, v_lif_wta, v_lif_sig = self.run_test(num_steps=num_steps,
                                                     num_neurons=num_neurons,
                                                     bias=bias,
                                                     theta=theta,
        ...
        spk_idxs_pre_inj = np.array([theta // bias]).astype(int) - 1
        ...
        spk_idxs_post_inj = np.array([inj_times[1] + (theta // bias) - 1 +
    ...
    def test_scif_fp_no_noise_interrupt_rfct_beg(self) -> None:
        ...
        total_period = theta // bias - neg_tau_ref
        num_epochs = 5
        num_steps = num_epochs * total_period + (theta // bias) + inj_times[1]
        v_scif, v_lif_wta, v_lif_sig = self.run_test(num_steps=num_steps,
                                                     num_neurons=num_neurons,
                                                     bias=bias,
                                                     theta=theta,
